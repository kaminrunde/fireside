...
const config: Config<Props, Result, State, DP> = {
  ...
  transformDispatch: {
    ...
    setWidth: (fn, sp, props) => (index, width) =>
      fn(props.mediaSize, index, width),
    setHeight: (fn, sp, props) => (index, height) =>
      fn(props.mediaSize, index, height),
    ...
    copyGridFrom: (fn, sp, props) => (mediaSize) =>
      fn(mediaSize, props.mediaSize),
