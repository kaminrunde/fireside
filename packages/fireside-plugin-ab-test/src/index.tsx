      ...
      component: (api) => {
        ...
        const deactivate = async () => {
          const result = await ctx.actions.alert({
            ...
            description:
              "This will complete remove the ab-test logic. The components will persist. Please remove the components you don't want any longer manually afterwars",
            options: ["ABORT", "OK"],
          });
          if (result === "OK") {
        ...
        return (
          <div>
            <button
              onClick={api.state ? deactivate : activate}
              style={styles.btn(Boolean(api.state))}
            >
              {api.state ? "on" : "off"}
            </button>
  ...
  ctx.extendGridRow({
    ...
    settingsModal: {
      ...
      component: function RowModalRow(api) {
        const setVariation = (v: "A" | "B" | "AB") => () => {
          ...
          const newState = produce(state, (state) => {
            ...
            if (v === "AB") {
              ...
              if (
                state.components[mediaSize]["A"].length === 0 &&
        ...
        const mode = state.components[mediaSize]?.A.includes(row)
          ? "A"
          : state.components[mediaSize]?.B.includes(row)
          ? "B"
          : "AB";
        ...
        return (
          <div style={styles.btnWrapper()}>
            <button
              style={styles.btn(mode === "A")}
              onClick={setVariation("A")}
            >
              A
            </button>
            <button
              style={styles.btn(mode === "B")}
              onClick={setVariation("B")}
            >
              B
            </button>
            <button
              style={styles.btn(mode === "AB")}
              onClick={setVariation("AB")}
            >
              Both
            </button>
          </div>
      ...
      return (
        <div>
          ...
          <div style={styles.staticWrapper()}>
            ...
            />
            <br />
            <button onClick={handleSubmit} style={styles.staticButton()}>
              Submit
            </button>
          </div>
        </div>
  ...
  ctx.onStoryUpdate((api) => {
    ...
    const newState = produce(api.state, (state) => {
      for (const ms in state.components) {
        ...
        state.components[ms].A = state.components[ms].A.filter(
          (n) => n <= maxRowIndex
        );
        state.components[ms].B = state.components[ms].B.filter(
          (n) => n <= maxRowIndex
        );
      }
