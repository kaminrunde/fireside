...
import createStaticComponent, {
  CreateStaticComponent,
} from "./createStaticComponent";
import createPage, { CreatePage } from "./createPage";
import onStoryUpdate, { OnStoryUpdate } from "./onStoryUpdate";
import extendComponentButtonList, {
  ExtendComponentButtonList,
} from "./extendComponentButtonList";
...
export default function createPlugin<State, Options extends t.PluginOptions>(
  ...
  return (options, actions) => {
    ...
    const context: PluginContext<State, Options> = {
      extendComponent: (config) => {
        events.push(...extendComponent(config, options));
      },
      extendGridRow: (config) => {
        events.push(...extendGridRow(config, options));
      },
      extendSettingsPage: (config) => {
        events.push(...extendSettingsPage(config, options));
      },
      extendComponentButtonList: (config) => {
        events.push(...extendComponentButtonList(config, options));
      },
      createPage: (config) => {
        events.push(...createPage(config, options));
      },
      createStaticComponent: (config) => {
        events.push(...createStaticComponent(config, options));
      },
      onStoryUpdate: (config) => {
        events.push(...onStoryUpdate(config, options));
      },
      options,
      actions,
    };
