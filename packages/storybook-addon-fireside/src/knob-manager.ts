...
function clearKnobs() {
  ...
  channel.emit(
    "storyboard-bridge/set-knobs",
    replaceFunctionsWithIdsAndEmit(currentKnobs)
  );
}
...
channel.on(
  ...
  async (component: t.Component) => {
    const selector = selectorStore[component.name];
    if (!selector) {
      throw new Error(
        'you forgot to implement "registerWidgetSelector" for widget ' +
          component.name
      );
    }
    clearKnobs();
...
const replaceOptionFunctions = (options: any) => {
  ...
  for (const key in options) {
    ...
    if (typeof value === "function") {
      ...
      for (const subKnob of value) {
        if (subKnob && subKnob.options) replaceOptionFunctions(subKnob.options);
      }
    }
  }
};
